...
const api = {
  ...
  native: {
    ...
    performanceTest: (iterations: number) => ipcRenderer.invoke('native:performanceTest', iterations),
    memoryTest: (size: number) => ipcRenderer.invoke('native:memoryTest', size),
    batchAdd: (operations: Array<{a: number, b: number}>) => ipcRenderer.invoke('native:batchAdd', operations),
  ...
  benchmark: {
    ...
    echoLarge: (text: string) => ipcRenderer.invoke('benchmark:echoLarge', text),
    
    // 计算测试
    add: (a: number, b: number) => ipcRenderer.invoke('benchmark:add', a, b),
    multiply: (a: number, b: number) => ipcRenderer.invoke('benchmark:multiply', a, b),
    ...
    // 性能分析
    getMemoryUsage: () => 
      ipcRenderer.invoke('benchmark:getMemoryUsage'),
    getCpuUsage: () => 
      ipcRenderer.invoke('benchmark:getCpuUsage')
  }
}
